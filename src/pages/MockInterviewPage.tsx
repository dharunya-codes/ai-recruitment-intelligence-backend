...
import { getSkillAssessmentQuestions } from '../data/mockData';
...
export const MockInterviewPage: React.FC = () => {
  ...
  const questions = getSkillAssessmentQuestions();
    ...
    return (
      <div className="space-y-6">
        <div className="bg-slate-800/80 border border-slate-700/70 rounded-2xl p-8 text-center shadow-lg">
          ...
          <p className="text-xs uppercase tracking-wider font-bold text-red-400">Skill Check Complete</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-8 text-sm">
            ...
            <div className="rounded-xl bg-slate-900/70 border border-slate-700 p-4"><strong className="block text-white">{questions.length - result.correct}</strong><span className="text-slate-400">Wrong Answers</span></div>
          </div>
        </div>
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-base font-bold text-white">Skill Gap MCQ Assessment</h2>
      ...
      <div className="rounded-xl border border-red-900/50 bg-red-950/40 p-4 text-sm text-red-100">
        These questions are based on skills identified as missing or weak in your resume analysis.
        <span className="block mt-1 text-xs text-red-200/80">Assessed skills: {questions.map((question) => question.skill).filter((skill, index, skills) => skills.indexOf(skill) === index).join(', ')}</span>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs text-slate-400">
        ...
        <div className="p-3.5 rounded-xl bg-slate-800/40 border border-slate-800 flex items-start gap-2.5">
          ...
          <div>
            <strong className="text-slate-300 block mb-0.5">Gap Coverage:</strong>
            <span>Questions target skills marked missing or weak in the analysis.</span>
          </div>
        </div>
