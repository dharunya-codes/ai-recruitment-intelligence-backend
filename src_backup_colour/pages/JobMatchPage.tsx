...
export const JobMatchPage: React.FC = () => {
  return (
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {sampleJobMatch.relevantProjects.map((proj) => (
            ...
            >
              ...
              <div className="pt-2 border-t border-slate-200/60 text-[11px] text-indigo-700 font-medium">
                {proj.impact}
              </div>
            </div>
          ))}
      ...
      <div className="bg-indigo-50/60 rounded-2xl border border-indigo-200 p-6">
        <div className="flex items-center gap-2 mb-3 text-indigo-950 font-bold">
          <Lightbulb className="w-4 h-4 text-indigo-600" />
          <span>Tailored AI Action Recommendations</span>
        </div>
        <div className="space-y-2.5 text-xs text-slate-700">
          {sampleJobMatch.recommendations.map((rec, idx) => (
            <div key={idx} className="flex items-start gap-2.5 bg-white p-3 rounded-xl border border-indigo-100">
